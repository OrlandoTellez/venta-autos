...
@section('content')
    ...
    <section>
        <article class="contenedor-texto">
            ...
            <x-boton icono="signo-mas" texto="Registrar Agencia" class="btn-registrarAgencia" />
        </article>

        <article>
            <div class="cards">

                <x-tarjeta-total titulo="Total de Agencias" valor="{{ count($agencias) }}" icono="clientes" />

                <x-tarjeta-total titulo="Nuevas este mes" valor="{{ count($agencias) }}" icono="clientes" />
                <x-tarjeta-total titulo="Agencias activas" valor="{{ count($agencias) }}" icono="clientes" />
                    ...
                    <tbody id="clientes-body">
                        @foreach ($agencias as $index => $agencia)
                            ...
                            <tr style="background-color: {{ $showColor }}; color: black;">
                                ...
                                <td>{{ $agencia->created_at->format('Y-m-d') }}</td>
                                <td>
                                    <form action="{{ route('agencia.delete', ['agencia' => $agencia->id]) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <div>
            ...
            <div class="modal-overlay">
                <div class="modal-content">
                    <div class="modal-header">
                        <h2 class="modal-title">Registrar Agencia</h2>
                    ...
                    <form class="register-form" action="{{ route('clientes.store') }}" method="POST">
                        ...
                        <label>
                            Nombre 
                            <input name="nombre" type="text" placeholder="Ingrese nombre de la agencia" required>
                        </label>

                        <label>
                            RFC
                            <input name="rfc" type="text" placeholder="Ingrese el RFC" required>
                        </label>

                        <label>
                            Direccion
                            <input name="direccion" type="text" placeholder="Ingrese la direccion" required>
                        </label>

                        <label>
                            Teléfono
                            <input name="telefono" type="text" placeholder="Ingrese el número" required>
                        </label>
        ...
        @push('scripts')
            <script>
                document.addEventListener('DOMContentLoaded', () => {
                    const $btnOpen = document.querySelector('.btn-registrarAgencia');
                    if (!$btnOpen) return;
                    ...
                    $btnOpen.addEventListener('click', () => {
                        ...
                        $form.addEventListener('submit', async (e) => {
                            ...
                            try {
                                const resp = await fetch("{{ route('agencias.store') }}", {
                                    method: 'POST',
                                    headers: {
